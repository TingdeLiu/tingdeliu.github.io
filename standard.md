...
> **摘录机制说明**：`excerpt` 字段优先作为文章列表的摘要文字。若未设置 `excerpt`，Jekyll 会使用 `<!-- more -->` 标记之前的内容作为摘录。推荐始终填写 `excerpt` 字段以确保摘要可控。
...
综述文章采用以下章节框架。**核心必选章节**（引言、基本概述、总结）不可缺少，其余章节根据领域灵活调整。一级标题使用阿拉伯数字编号。
...
> **示例**：Robot-Navigation 综述按技术栈分章（感知→定位→建图→规划→控制）；AI-Agent 综述包含记忆机制、安全等独立章节。领域特性决定章节组织方式。
...
3. **标题层级与序号**：
   - `#` 一级标题（主章节）：使用阿拉伯数字序号，格式 `# 1. 引言`、`# 2. 基本概述`……
   - `##` 二级标题：使用层级编号，格式 `## 1.1 为什么需要…`、`## 2.1 核心要素`……父章节号.子序号
   - `###` 三级标题（具体方法/技术）：无需序号，直接写方法名，不在标题中附加英文翻译
   - `####` 四级标题（仅在系统架构模块等特殊场景使用）
   - **标题中不附加英文翻译**：英文缩写/名称可保留，但括号内的中文或英文释义不写在 `#`/`##` 标题内（首次出现的中文翻译放在正文中说明）
   - > 注：本规范文档自身的子节编号（如 4.1、5.1）采用跨章节连续编号便于引用，不遵循上述重置规则。
...
- [ ] 引言末段有"本文旨在系统梳理..."句式
- [ ] 有发展时间线（Mermaid 图）（如该领域适用）
- [ ] 所有图片使用 `<div align="center">` 格式
- [ ] 首次出现的英文缩写提供中文翻译
- [ ] 主流数据集章节存在
- [ ] 文末有"总结"章节
- [ ] Front matter 中 `toc: true` 已设置（站点自动生成侧边栏目录，无需手动插入 `{:toc}`）
- [ ] 文章标签不超过 8 个
