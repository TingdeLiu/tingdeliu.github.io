...
**Images** (`images/`): Post images organized by topic subdirectories. Existing topics: `vln/` (LLM/VLM-based navigation), `robotics_navigation/` (classical navigation without LLM/VLM), `vla/` (Vision-Language-Action, manipulation), `vlm/`, `wm/` (world models, 3D reconstruction/generation), `si/` (spatial intelligence, 3D scene understanding), `agent/`, `llm-training/`, `DL/` and `ML/` (general concept diagrams for teaching posts, not paper figures). `ros2/` is referenced by the paper-summary skill but not yet created. The paper-summary skill moves renamed figures into these subdirectories.
