...
- **从反馈学习**：
  ...
  - CLIP-RT：语言反馈引导
  - π*₀.₆：从专家干预中学习（详见[经典论文第8.12节](#8-12-pi06-2025)）
...
- **π*₀.₆**：整合人类干预的在线学习（详见[经典论文第8.12节](#8-12-pi06-2025)）
...
| 模型 | 动作解码器 | 参数规模 | 特点 |
|------|-----------|---------|------|
| **OpenVLA** | 离散token化 | 7B | 首个开源大规模VLA（[详见第8.9节](#8-9-openvla-2024)） |
| **π₀** | Flow Matching | 3B (VLM) + 860M (Action) | 50Hz实时控制（[详见第8.10节](#8-10-pi0-2024)） |
| **Diffusion Policy** | DDPM | - | 多模态动作分布（[详见第8.5节](#8-5-diffusion-policy-2023)） |
...
| 模型 | 高层规划 | 低层执行 | 特点 |
|------|---------|---------|------|
| **π₀.5** | 增强推理VLM | Flow Matching | 多步骤推理链（[详见第8.11节](#8-11-pi05-2025)） |
| **RT-H** | VLM子任务分解 | RT-1 | 层级任务执行（RT-1详见[第8.1节](#8-1-rt-1-2022)） |
...
### 2. 基于世界模型的VLA：数据飞轮的引擎

世界模型（World Models）被认为是机器人学习的“System 2”（慢思考）。如果VLA是“看到就做”的条件反射（System 1），世界模型则允许机器人在脑海中“想象”行动后果后再执行。近期，世界模型正逐渐成为合成数据的终极生成器，转动了VLA的“数据飞轮”。

**代表性里程碑与研究方向**：
- **统一仿真与真实数据**：如Google的 **UniSim**，首次证明可以将3D仿真、真实机器人数据和互联网视频混合，训练出统一的世界模拟器。
- **产业级生成平台**：NVIDIA 的 **Cosmos** 平台，为具身智能专门设计，能够生成符合物理规律的视频轨迹，极大地缓解了真实数据短缺的瓶颈。
- **合成数据驱动VLA训练**：如 **GIGA-star** 等工作，直接利用世界模型生成的数据来训练VLA，甚至证明了模型生成的合成数据对策略训练的效果提升可以达到30%。
- **未来方向**：短期内世界模型将作为数据增强工具，中长期大概率会与VLA深度融合，成为通用物理智能的大脑。
