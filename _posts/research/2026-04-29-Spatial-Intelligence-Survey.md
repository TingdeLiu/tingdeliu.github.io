...
> 💡 **知识体系与关联阅读**：
> 本篇聚焦于**三维几何感知、神经三维重建（NeRF/3DGS）与空间推理（3D-LLM / Spatial VLM / 具身感知）**。
> 若您关注 **通用多模态大模型基座（ViT 编码器演进、跨模态 Projector/Q-Former 对齐、多模态预训练与后训练全流程）**，请阅读基座篇综述：[《VLM综述：多模态融合方法全景》](/VLM-Survey/)。
...
- **ChatCaptioner based Prompting**: 利用多视图 [BLIP-2](/VLM-Survey/#84-blip-22023) 对 3D 场景进行问答，汇总信息后由 GPT 生成详细描述。
...
### 精华
- 证明了直接在成熟的 2D 多模态大模型（如 [LLaVA](/VLM-Survey/#85-llava2023) 系列）上通过添加 3D 位置编码与联合微调，即可高效实现强大的 3D 场景理解，无需从头训练 3D 编码器。
- 巧妙利用 RGB-D 深度图与相机内外参，将 2D视觉特征（[CLIP](/VLM-Survey/#82-clip2021) Patches）与 3D 空间绝对坐标进行加法融合，构建极简的 “3D Patches” 表征。
...
### 精华
1. **去特化 3D 设计**：证明了标准 VLM（如 [Qwen3-VL](/VLM-Survey/#qwen3-vl)）在没有任何架构修改、辅助编码器、重度数据增强以及回归损失的前提下，仅依靠纯文本 SFT 就能成为极具竞争力的 3D 学习器。
...
### 精华
1. **首个在线三维多模态大模型**：突破了传统 3D MLLM 必须输入完整场景或预先剪辑视频的离线限制，实现了直接在实时视频流上的实时三维空间理解与动态交互。
2. **自回归流式决策控制**：将“何时响应”与“何时保持静默”建模为大模型的下文 Token 预测任务（引入 `<SEP>` 和 `<END>` Token），无需重复编码历史帧，大幅降低计算延迟。
3. **轻量级几何先验注入**：利用单目视频三维重建模型（StreamVGGT）增量提取相机参数与潜在三维几何特征，通过交叉注意力机制无缝融合进 2D 视觉流，摆脱对昂贵三维点云传感器的依赖。
4. **几何自适应体素压缩（GAVC）**：针对流式长视频带来的视觉 Token 冗余问题，通过三维反投影和三维空间 K-Means 聚类，自适应压缩三维空间内的冗余体素，在保留结构完整性的同时降低 KV 缓存负担。
5. **百万级流式 3D QA 数据集与基准**：构建了包含 100 万问答对的 Stream3D-1M 数据集及涵盖 29 种任务的 Stream3D-Bench 基准，为流式三维理解提供了首个系统化评测平台。

---

### 1. 研究背景/问题
现有的三维多模态大模型（3D LMMs）主要运行在**离线（Offline）模式**下。它们在与用户交互之前，必须接收完整的 3D 场景观测（如 3D 点云、Mesh 网格）或者一段预先剪辑好的静态视频。这种“先看完、后回答”的范式无法满足具身智能、AR/VR 智能眼镜等实际应用中对**流式实时交互**的需求，例如机器人在移动过程中需要实时识别并避开障碍物、估计物体的物理尺寸，或者在目标物体出现时主动发出提醒。

虽然在 2D 视频理解领域已有部分流式视频大模型（如 VideoLLM-online），但它们缺乏对三维世界物理几何结构的建模（如物体间的三维距离、物理大小、相机的真实三维轨迹等）。即使在这类 2D 流式大模型上进行大规模 3D 数据微调，其三维空间推理能力依然较弱。因此，迫切需要设计一种能够同时兼顾**流式实时决策**与**三维空间几何建模**的通用三维多模态大模型。
...
*   **自回归流式控制建模 (Streaming Control Modeling)**
    *   **输入**：当前帧图像、历史上下文、用户查询。
    *   **处理**：在大模型词表中引入两个特殊的决策控制 Token：`<SEP>`（Streaming Continuation，代表流式继续，模型保持静默，继续读取下一帧图像）和 `<END>`（Response Trigger，代表触发响应，模型停止读取新帧并开始输出文本答案）。
    *   **输出**：决策 Token（`<SEP>` 或 `<END>`）以及文本响应内容。
...
## 6.27 Qwen-3D (2026) {#6-27-qwen-3d}
———通用 3D 视觉语言多模态大模型，以世界坐标系自注意力与无瓶颈 Query 掩码解码统一 2D/3D 空间定位与推理

📄 **Paper**: [arXiv:2608.02980](https://arxiv.org/abs/2608.02980) · [Project Page](https://qwen-3d.github.io/)

### 精华
1. **打破 3D 定位信息瓶颈**：颠覆了传统 3D-LLM 将空间定位局限于文本坐标生成或单一 `<REF>` 特殊标记的落后做法，首创将 VLM 主干输出的稠密多模态特征无损接入 Mask2Former 式 Query 掩码解码器。
2. **世界坐标系体素特征池化**：利用相机位姿与深度将多视角图像特征反投影到统一三维世界坐标，并执行 5cm 体素池化合并跨视角冗余，支持单场景端到端摄入 90+ 帧真实传感器 RGB-D 视频流。
3. **频率对齐的 3D RoPE**：将 2D 多模态旋转位置编码扩展为空间三轴频率均匀分布的 3D RoPE，赋予自注意力机制原生度量感知能力，配合全双向注意力消除点云排列歧义。
4. **2D/3D 联合训练保持通用性**：仅引入约 50M 可训练参数（LoRA 微调注意力层 + 训练掩码解码器），在 3D 定位与分割大幅刷新 SOTA 的同时，零损耗保持 Qwen2.5-VL 强大的 2D 多模态问答与接地能力。

---

### 1. 研究背景/问题
多模态大模型（LMM）在单图和短视频理解上取得了突破，但在处理长时程多视角视频流时面临严峻挑战：一方面受限于注意力机制的二次方复杂度与有限上下文窗口；另一方面，图像像素平面的二维表征缺乏跨视角的空间度量一致性。

引入 3D 几何（深度与相机位姿）能将多视角观测天然压缩至持久的三维世界坐标系中。然而，现有 3D-LLM 在 3D 定位（Visual Grounding）与实例分割任务上仍大幅落后于专用 3D 感知模型。其根本瓶颈在于**解码机制的信息损失**：传统方法要么在文本空间预测离散坐标（缺乏连续几何感知且存在歧义），要么依赖外部检测器生成 Proposal（受限于检测器召回率），要么仅通过单一 `<REF>` 标记传递信息（形成严重特征瓶颈）。如何构建一个兼具长时程几何压缩、原生 3D 空间推理与无损稠密掩码预测的通用 3D 视觉语言模型，是当前具身与空间智能的核心痛点。
...
#### ① 整体框架概述
Qwen-3D 基于 Qwen2.5-VL 基础模型构建，由**几何感知场景表征**、**世界坐标系视觉-语言注意力**以及**多任务双分支解码器**三大核心模块构成：
- **几何表征模块**：将多视角 2D 图像特征与深度/位姿反投影至统一世界坐标系，通过体素池化去除视角冗余；
- **3D RoPE 注意力模块**：利用均匀频率分布的 3D 旋转位置编码和全双向注意力，在世界空间内联合推理点云与文本；
...
**1. 几何感知场景表征与体素压缩（Geometry-Aware Scene Representation）**
- **输入**：多视角 RGB 图像序列、对应深度图及相机内外参。
- **处理**：首先通过冻结的 Qwen2.5-VL 视觉编码器（ViT）提取各视角 2D 特征图；接着利用深度与相机位姿将像素特征反投影（Unproject）到统一世界坐标系，得到特征-三维坐标对：
  $$FC = \{ (f_i, p_i) \}_{i=1}^M \quad (f_i \in \mathbb{R}^D, \; p_i \in \mathbb{R}^3)$$
  为消除同一物理表面在多视角下的重复采样，算法在三维空间划分 5cm 体素网格，对落入同一体素内的特征向量与坐标执行均值池化（Mean-Pooling）。
- **输出**：紧凑且保留空间几何结构的无序三维点特征集合：
  $$FC' = \{ (f_j, p_j) \}_{j=1}^{M'} \quad (M' \ll M)$$
- **设计动机**：直接将多视角像素输入 Transformer 会引发显存爆炸；体素池化将多视角冗余大幅压缩至持久几何结构中，使推理期可一次性处理多达 90 帧 RGB-D 输入。

> **举个例子（体素池化算账）**：
> 假设一个室内场景输入 90 帧图像，每帧产生约 1,000 个有效深度像素，朴素拼接共有 90,000 个视觉 token，注意力矩阵将产生高达 $90,000^2 \approx 81$ 亿项计算，直接导致 GPU 显存耗尽（OOM）；
> 经过 5cm 体素池化后，多视角重叠点被聚合成单一物理点，token 数量被压缩至约 4,000 到 8,000 个（压缩比达 10 到 20 倍），注意力计算量缩减至原先的 $\frac{1}{100}$ 以下，使长视频流端到端推理在 2 秒内即可完成。

**2. 3D 旋转位置编码与全双向注意力（Geometry-Aware Vision-Language Attention）**
- **输入**：体素化 3D 视觉特征序列 $FC'$ 与用户自然语言 Prompt token 序列：
  $$T = \{ t_k \}_{k=1}^L \quad (t_k \in \mathbb{R}^D)$$
- **处理**：
  - **3D RoPE 构建**：扩展 Qwen 的 Multimodal RoPE 为四元分量位置编码：
    $$PE_{3D}(p) = [PE(t); PE(x); PE(y); PE(z)]$$
    文本 token 沿序列时间轴 $t$ 递增（退化为标准 1D RoPE），点云 token 共享固定时间 ID，并以其实际三维世界坐标 $(x, y, z)$ 作为空间位置 ID。在通道频率分配上，为 $x, y, z$ 各轴均匀分配高低频段，避免了传统多模态位置编码中时间轴独占高频导致的 3D 轴向空间分辨率失真。
  - **全双向注意力替换**：将 2D VLM 原生的因果自回归掩码（Causal Mask）替换为**全双向注意力（Full Attention Mask）**，允许所有视觉与文本 token 在单次前向传播中互相交互。
- **输出**：融合三维空间几何与语义理解的上下文视觉表征与语言表征：
  $$V = \{ v_i \}_{i=1}^{M'}, \quad T = \{ t_j \}_{j=1}^L$$
- **设计动机**：3D 点云本质上是无序集合，因果掩码施加的伪时序强行打乱了空间对称性；全双向注意力配合 3D RoPE 使模型具备精确的世界坐标度量距离感知。

**3. 无瓶颈几何感知掩码解码（Geometry-Aware Decoding）**
- **输入**：主干网络输出的视觉特征 $V$、文本特征 $T$ 以及 $N$ 个可学习的三维物体查询向量：
  $$Q = \{ q_n \}_{n=1}^N$$
- **处理**：
  - 采用 Mask2Former 架构的解码层，交替执行查询向量之间的自注意力（Self-Attention）以及对视觉与文本特征的交叉注意力（Cross-Attention）。
  - 解码后，每个物体 Query 通过与更新后的视觉 token 点积预测**三维/二维分割掩码（Segmentation Mask）**；同时通过与文本 token 点积计算**文本跨度关联分数（Language Grounding Score）**。
  - 文本生成分支则沿用 Qwen 语言头，基于上下文表征进行自回归 Next-Token 预测。
- **输出**：目标物体的 3D 点云分割掩码、对应的边界框（通过掩码极值点转换）以及开放式自然语言回答。
- **设计动机**：摒弃传统将定位压缩为单个 `<REF>` token 或文本坐标的瓶颈设计，使密集预测任务能够直接借力主干网络提取的高维上下文视觉语义。

```mermaid
graph TD
    subgraph "输入与多视角反投影"
        A["多视角 RGB 图像"] --> B["冻结 Qwen2.5-ViT"]
        B --> C["2D 特征图"]
        D["深度图 + 相机位姿"] --> E["3D 反投影"]
        C --> E
        E --> F["空间特征坐标对 (f, p)"]
    ...
    subgraph "几何压缩与多模态注意力"
        F --> G["5cm 体素池化 (Voxel Pooling)"]
        G --> H["紧凑 3D 点云 Token"]
        I["用户 Prompt 文本"] --> J["文本 Tokenizer"]
        H & J --> K["Qwen VL 注意力层 (LoRA 微调)"]
        L["空间对齐 3D RoPE"] --> K
    end

    subgraph "双分支解耦输出"
        K --> M["全量视觉特征 V + 语言特征 T"]
        M --> N["Mask2Former 掩码解码器"]
        O["可学习 Object Queries"] --> N
        N --> P["3D/2D 实例分割与空间定位 (Mask & Box)"]
        M --> Q["Qwen 语言生成头"]
        Q --> R["开放式 VQA 文本回答"]
    end
```
...
1. **掩码损失 $L_{mask}$**：通过匈牙利算法（Hungarian Matching）将预测掩码与真实物体实例进行二分图匹配，采用二值交叉熵（BCE）与 Dice Loss 监督匹配掩码；
2. **文本跨度接地损失 $L_{textground}$**：以 BCE 监督匹配查询向量与文本描述中对应短语 token 的对应关系，未匹配查询被抑制为低概率；
3. **文本生成损失 $L_{gen}$**：仅对开放式问答与描述任务计算 token 级交叉熵。

**高效参数微调策略**：全模型仅引入约 **50M** 可训练参数。视觉编码器（ViT）完全冻结，多模态注意力层采用 LoRA 进行高效微调，掩码解码器从零开始初始化。在 8 张 48GB L40S GPU 上，混合 2D 数据（RefCOCO、COCO、LLaVA-Instruct、Alpaca）与 3D 数据（SR3D、NR3D、ScanRefer、ScanNet200、ScanQA、SQA3D）联合训练 200k 步（约 3 天）。为强化跨模态对齐，训练期还以概率 $p$ 借助 MoGE 等单目重建模型将 2D 图像实时升维至伪 3D。

---

### 3. 核心结果/发现

1. **3D 视觉定位（3D Referential Grounding）刷新纪录**：
   在真实传感器原始 RGB-D 点云评测中（非人造网格），Qwen-3D（3B）在 ScanRefer 上 Top-1@0.25 达到 53.5%，在 SR3D / NR3D 上全面大幅领先以往通用多模态模型（LLaVA-3D、Grounded-3D-LLM），并战胜了专有 3D 定位专家模型（UniVLG、BUTD-DETR）。
2. **3D 实例分割性能飞跃**：
   在挑战性的 ScanNet200 实例分割基准上，Qwen-3D 取得 25.3% mAP 与 41.5% mAP25，相较此前唯一涉足该任务的 3D LMM 基线（Grounded-3D-LLM，12.1% mAP）取得超一倍的巨幅提升，首次让通用大模型具备了逼近专家级模型（ODIN、UniVLG）的密集 3D 分割能力。
3. **3D 问答与场景推理（3D VQA）**：
   在 ScanQA 与 SQA3D 问答榜单上，Qwen-3D-3B 全面超越专家模型 UniVLG；7B 变体进一步刷新各项指标至全场最高。
4. **2D 多模态基础能力零退化**：
   得益于 2D/3D 联合训练策略，Qwen-3D 在 RefCOCO/+/g 2D 定位与 RealWorldQA 真实世界评测中分别取得 88.1% / 82.7% / 84.0% 与 60.4% 的高分，与原始 Qwen2.5-VL-3B（89.1% / 82.4% / 85.2% / 62.6%）几乎完全一致，彻底克服了灾难性遗忘。
5. **消融实验关键结论**：
   - **全量特征 vs `<REF>` Token**：将接口退化为单一 `<REF>` 标记后，准确率从 53.5% 断崖式跌落至 39.7%（-13.8%），证明稠密全量特征对 3D 几何解码的决定性作用；
   - **全双向注意力 vs 因果掩码**：全双向注意力准确率达 53.5%，因果掩码仅有 36.4%（-17.1%），证实点云空间无序性下因果掩码的严重弊端；
   - **频率对齐 3D RoPE**：对齐版 3D RoPE（53.5%）显著优于未对齐的 Vanilla 3D RoPE（49.9%）与 2D RoPE（53.2%）；
   - **主干微调**：完全冻结 VLM 主干导致准确率暴跌至 37.0%，验证了 LoRA 跨模态空间注意力自适应的必要性。

---

### 4. 局限性
1. **静态场景假设**：当前模型基于单一静态全局坐标系建模，难以直接处理包含物体非刚性形变、移动交互及拓扑变迁的动态复杂 4D 场景。
2. **单步接地约束**：评测任务主要局限于单步定位与问答，尚不具备多步时空依赖比较与长程多轮交互式具身推理规划能力。
3. **依赖外部几何质量**：三维空间表征完全构建于外部估计的深度图与相机位姿之上，位姿漂移与深度伪影会直接传导并限制最终的下游定位精度。
