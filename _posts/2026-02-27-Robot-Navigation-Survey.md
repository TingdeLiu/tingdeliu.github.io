...
## 2.3 传感器外参标定（Extrinsic Calibration）
...
## 3.2 EKF 定位（Extended Kalman Filter Localization）

**直觉理解**：想象你蒙眼走路，靠步数和转弯角度估计自己的位置（这是"预测"）；每当你摘下眼罩瞥一眼地图上的路标，就用路标位置来修正你的估计（这是"更新"）。EKF 做的就是这件事的数学版本。
...
## 3.3 UKF 定位（Unscented Kalman Filter Localization）

**与 EKF 的区别**：EKF 用泰勒展开对非线性函数做一阶线性化近似，在高非线性系统中误差较大。UKF 则通过精心选取的**Sigma 点集**来近似非线性变换后的概率分布，无需求导，精度更高。
...
## 3.4 粒子滤波定位（Particle Filter / AMCL）
...
# 6. 路径跟踪（Path Tracking / Control）

路径规划给出了一条理想路径，而**路径跟踪控制器**的任务是让机器人实际跟随这条路径运动。由于真实世界存在噪声、模型误差和外界干扰，控制器需要实时计算修正量。

## 6.1 Pure Pursuit（纯追踪控制）

**直觉理解**：想象你开车，目光盯着前方一个固定距离（**预瞄距离 Look-ahead Distance $L_d$**）的目标点，不断调整方向盘朝它转。这就是 Pure Pursuit 的思路。
...
## 6.2 自适应 Pure Pursuit（Adaptive Pure Pursuit）
...
## 6.3 后轮反馈控制（Rear Wheel Feedback）

以车辆**后轴中点**为跟踪参考点（而非前轴或重心），直接消除后轮在参考路径上的横向误差和航向误差。相比 Pure Pursuit 有更严格的数学收敛保证。
...
## 6.5 LQR 路径跟踪（线性二次调节器，Linear Quadratic Regulator）

**思路**：将路径跟踪问题建模为**最优控制问题**。在车辆线性化模型下，LQR 求解最小化如下代价函数的最优控制律：
...
## 6.6 MPPI（模型预测路径积分，Model Predictive Path Integral）

**思路**：属于**模型预测控制（MPC）** 的随机变体。在当前时刻，向前采样**大量随机控制序列**（通过 GPU 并行采样），用运动模型仿真每条轨迹的未来状态，根据轨迹代价（碰撞 + 偏离路径 + 控制平滑）计算**加权平均**作为当前控制输出，然后滑动时间窗口重复。
...
> 关于视觉语言导航（VLN）和 VLA 大模型的内容，请参考本站 [VLN综述](/VLN-Survey/) 和 [VLA综述](/VLA-Survey/) 系列文章。
