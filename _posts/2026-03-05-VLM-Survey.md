...
这一节我们将详细介绍经典的 VLM 三阶段训练范式（5.1）、偏好对齐技术（5.2）、训练中涉及的关键支撑技术（5.3）；随后转向实操视角——超参数如何设置与调整（5.4）、训练过程中如何监控指标与解读 loss 曲线（5.5）、训练出问题时如何排查（5.6）；最后对 Qwen-VL 系列模型的训练演进路径进行深度案例剖析（5.7）。

---

## 5.1 经典三阶段训练范式 (Three-Stage Training Pipeline)

现代大语言模型驱动的 VLM（如 LLaVA、MiniGPT-4、InternVL 等）通常遵循一个经典的**三阶段训练 pipeline**。这一流水线通过逐步解冻参数和优化不同规模与质量的数据集，使得视觉与语言表征能够层层对齐。
...
## 5.2 偏好对齐与后训练 (Preference Alignment & Post-Training)
...
## 5.3 核心技术关键点 (Key Training Technologies)
...
## 5.4 训练超参数实战指南 (Hyperparameter Tuning in Practice)
...
一个实用的直觉：**学习率应与"参数当前的质量"成反比**。对齐阶段投影层的学习率（1e-3）是 SFT 阶段 LLM 学习率（2e-5）的 50 倍，正是因为前者从零开始、后者只需"轻微修正"。InternVL 等工作还对 ViT 进一步使用**逐层学习率衰减**（layer-wise lr decay，衰减系数约 0.9~0.95）：越靠近输入的层学到的特征越通用，越应该少动。
...
LLaVA-NeXT 在解冻 ViT 时为 vision tower 单独设置 **2e-6** 的学习率（约为 LLM 的 1/10），这一"ViT 用十分之一学习率"的惯例被后续大量开源工作沿用。

**Qwen-VL 的三阶段配置**（论文 arXiv:2308.12966）则代表"工业级大规模训练"的取向：

| 超参数 | 阶段一：预训练 | 阶段二：多任务预训练 | 阶段三：SFT |
|--------|--------------|--------------------|------------|
| 峰值学习率 | 2e-4 | 5e-5 | 1e-5 |
| 全局 batch size | 30720 | — | 128 |
| 优化器 | AdamW（β₁=0.9，β₂=0.98，eps=1e-6） | 同左 | 同左 |
| weight decay | 0.05 | 0.05 | 0.05 |

两份配方的差异本身就是教科书：**训练数据规模决定正则强度**。LLaVA 用几十万精选数据训 1 epoch，weight decay 设 0 即可；Qwen-VL 一阶段要消化 14 亿噪声图文对，weight decay 提高到 0.05，并把 AdamW 的 β₂ 从默认的 0.999 调低到 0.98——二阶矩估计对梯度分布变化反应更快，能显著降低大 batch 训练中 loss 尖刺的风险。

### 3. batch size 与学习率的联动

显存不够时人们首先想到压缩 batch size，但 **batch size 变了，学习率必须跟着变**：

$$\eta' = \eta \cdot \frac{B'}{B} \;\text{（线性缩放，预训练场景）}, \qquad \eta' = \eta \cdot \sqrt{\frac{B'}{B}} \;\text{（平方根缩放，微调场景更稳妥）}$$

- 等效全局 batch = 单卡 batch × 梯度累积步数 × GPU 数。**梯度累积与直接增大 batch 在数学上等价**，是显存受限时维持原配方有效性的首选手段。
- 例：照抄 LLaVA-1.5 配方（batch 128、lr 2e-5），但你只能开到全局 batch 32——学习率应下调到约 1e-5（平方根缩放）甚至 5e-6（线性缩放），否则训练会明显震荡。
- 缩放规则在 batch 跨度过大（超过约 8 倍）时失效，此时建议重新扫学习率。

### 4. warmup、衰减与 epoch 数

**为什么必须 warmup？**训练初期 Adam 的二阶矩估计只基于极少数样本，极不可靠；同时随机初始化的投影层会向预训练权重回传"垃圾梯度"。线性 warmup（常用 ratio 0.03，即总步数的 3%）让模型在学习率很小的阶段先把最离谱的参数修正掉，再进入全速学习。

**衰减到哪里？**余弦衰减到 0 是最常见选择，部分工作衰减到峰值的 10%。注意训练末期 loss 会随学习率衰减再下探一截（见 5.5 节图），这是调度器的正常效果，不代表"还能再训很久"。

**训几个 epoch？**SFT 的主流选择是 **1~2 个 epoch**：指令数据规模小、格式统一，多训几轮模型很快从"学习能力"滑向"背诵答案"，验证集 loss 普遍在第 2~3 个 epoch 开始回升（过拟合，见 5.5 节异常模式图 (d)）。对齐预训练与多任务预训练几乎都只过 1 epoch——大规模数据下"见更多新数据"永远优于"重复旧数据"。

### 5. 数据配比：防止文本能力退化

多模态训练会天然侵蚀 LLM 的纯文本能力，两个常用对策：

1. **混入纯文本数据**：SFT 阶段混入约 10%~30% 的纯文本指令数据（LLaVA-1.5 的 665K 指令数据中就包含 40K ShareGPT 纯文本对话；Qwen2-VL、InternVL2 同样在各训练阶段保留纯文本语料）。
2. **逐 token 损失重加权**：多模态样本的序列远长于纯文本样本，朴素的逐 token 平均会让多模态梯度主导训练。Qwen3-VL 的 square-root reweighting（对每样本的 token 数做平方根归一化）正是针对这一问题（见 7.8 节）。
...
| Gradient Checkpointing | 不存储中间激活，反向传播时重算 | 计算量增加约 30% |
| DeepSpeed ZeRO-2 / ZeRO-3 | 优化器状态/梯度/参数分片到多卡 | 通信开销增加 |
...
## 5.5 训练监控与 Loss 曲线解读 (Monitoring & Reading Loss Curves)
...
- **记录 token 级准确率**作为 loss 的补充：即预测下一 token 的 top-1 命中率，对 SFT 阶段尤其直观。
...
关于 loss 尖刺，大模型训练史上有不少公开经验可以借鉴：**PaLM** 训练中遇到尖刺的标准操作是回退 checkpoint 并跳过引发问题的几百个 batch——同样的数据单独拿出来训练并不会复现问题，说明尖刺是"参数状态 × 特定数据"的组合事件；**GLM-130B** 通过缩小 embedding 层梯度（缩放系数 0.1）显著减少了尖刺；PaLM 还引入 **z-loss** 正则项抑制 logits 漂移。VLM 在第二阶段的大规模联合训练中最容易遇到这类问题。
...
- **预警信号**：grad norm 先于 loss 持续抬升——这通常是发散的前兆，此时降学习率还来得及；
- **梯度裁剪 max_norm=1.0** 是几乎所有 VLM 训练的标配兜底。但若 grad norm **长期贴着裁剪阈值**，说明学习率设大了——裁剪只应偶尔触发，不应常态化。
...
## 5.6 常见训练问题排查 (Troubleshooting)

把 5.4、5.5 节的要点收敛为一棵决策树，训练出问题时按图索骥：
...
flowchart TD
    ...
    F -->|是| F1["① 混入 10%~30% 纯文本指令数据\n② 降低 LLM 学习率或改用 LoRA\n③ 清洗编造性 caption 数据\n④ 引入 DPO 偏好对齐抑制幻觉（见 5.2）"]:::fix
    F -->|否| G["对照 5.5 节异常模式图逐项排查"]:::fix
```
...
4. **评测驱动开发**：不要等训练完才评测。任何配置改动（数据、超参、模板）都应先在小规模代理实验上验证（见 5.4 节调参工作流），确认曲线与小评测正常后再上全量。

---

## 5.7 案例剖析：Qwen-VL系列训练演进 (Case Study: Evolution of Qwen-VL Training)

阿里巴巴开源的 Qwen-VL（千问视觉大模型）是多模态领域的标杆性工作。从 Qwen-VL 到 Qwen2-VL，再到 Qwen2.5-VL，其训练策略的演进路径清晰地反映了多模态模型技术的发展趋势。

```mermaid
flowchart TD
    %% Timeline implemented as flowchart
    classDef qwen fill:#eff6ff,stroke:#3b82f6,stroke-width:2px,color:#1e3a8a;

    V1["Qwen-VL (2023)\n【经典三阶段训练】"]:::qwen --> V2["Qwen2-VL (2024)\n【架构与分辨率升级】"]:::qwen --> V3["Qwen2.5-VL (2025)\n【超大参数与推理强化】"]:::qwen

    subgraph Details1 ["Qwen-VL 关键特征"]
        D1_1["1. 视觉编码器与LLM双冻结 (Stage 1)"]
        D1_2["2. 视觉编码器参与联合训练 (Stage 2)"]
        D1_3["3. 视觉编码器冻结进行 SFT (Stage 3)"]
        D1_4["4. 采用单层 Cross-Attention 桥接"]
    end

    subgraph Details2 ["Qwen2-VL 关键特征"]
        D2_1["1. 原生动态分辨率 (Naive Dynamic Resolution)"]
        D2_2["2. 引入 3D 卷积提取视频 Tube 特征"]
        D2_3["3. 采用 3D M-RoPE 进行时空对齐"]
        D2_4["4. 训练阶段：Stage 1 仅训练 ViT; Stage 2 联合训练; Stage 3 冻结 ViT 进行 SFT"]
    end

    subgraph Details3 ["Qwen2.5-VL 关键特征"]
        D3_1["1. 4.1万亿 Token 超大规模预训练"]
        D3_2["2. 升级版 mRoPE 绝对时间维度对齐"]
        D3_3["3. Window Attention 大幅降低高分辨率计算量"]
        D3_4["4. 后训练引入 DPO 与 GRPO 进行视觉推理对齐"]
    end
...
*   **架构特征**：
    ...
    - LLM：Qwen-7B。
    - 连接器：单层 Cross-Attention 模块（将 ViT 输出的 256 维特征映射为固定 256 个 visual tokens）。
*   **三阶段训练策略**：
    - **Stage 1 (预训练/特征对齐)**：
      - **冻结策略**：**冻结 ViT 和 LLM**，仅训练 Cross-Attention 连接器。
      - **训练数据**：14 亿图文对。
      - **目的**：打通图像特征和文本语义的连接通道。
    - **Stage 2 (多任务预训练)**：
      - **冻结策略**：**全参数解冻**，ViT、连接器和 LLM 同时参与训练。
      - **训练数据**：7 亿对高质量多模态数据，分辨率提升至 $448 \times 448$。引入边界框坐标标注进行 Grounding（目标定位）训练；同时加入大规模 OCR 数据。
      - **目的**：极大地丰富模型的定位 and 细粒度感知能力。
    - **Stage 3 (SFT/指令对齐)**：
