...
**Qwen2.5-VL** 通过 3D-RoPE 和动态帧率采样支持数十分钟的超长视频；**Qwen3-VL** 进一步以显式文本时间戳与 Interleaved MRoPE 将多模态上下文扩展到 256K（30分钟视频内 Needle-in-a-Haystack 100% 准确率，见 7.10 节）。
...
2. **逐 token 损失重加权**：多模态样本的序列远长于纯文本样本，朴素的逐 token 平均会让多模态梯度主导训练。Qwen3-VL 的 square-root reweighting（对每样本的 token 数做平方根归一化）正是针对这一问题（见 7.10 节）。
