...
| 输入类型 | 示例 |
|:---|:---|
| 文本提示（Text） | "一个沉没的水下古城，阳光从水面折射进来" |
| 单张图像（Image） | 室内照片、风景图、艺术插画 |
| 视频片段（Video） | 短视频或 360° 全景视频 |
| 粗糙 3D 布局（Coarse 3D Layout） | 草图式的空间配置 |

**3DGS 作为世界表示：选型的核心逻辑**

Marble 的核心技术选型为 **3D 高斯泼溅（3D Gaussian Splatting, 3DGS）**。与 NeRF 相比，3DGS 在世界模型场景下具有显著优势：

```mermaid
flowchart LR
    Input["多模态输入\nText / Image / Video / 3D Layout"] --> LWM

    subgraph LWM["大型世界模型 (LWM)"]
        direction TB
        Enc["多模态编码器\n语义理解与空间推理"]
        Gen["3D 世界生成器\n空间一致性建模"]
        Enc --> Gen
    ...
    LWM --> GS["3D Gaussian Splats\n半透明粒子集合\n（可实时渲染）"]

    GS --> View["自由视角导航\nFree-viewpoint Navigation"]
    GS --> Edit["交互式编辑\nInteractive Editing"]
    GS --> Export["多格式导出\nGaussian splat / Mesh / Video"]
    GS --> API["World API\n嵌入游戏引擎 / 机器人仿真"]
```
