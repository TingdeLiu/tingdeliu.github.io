...
<div align="center">
  <img src="/images/wm/World_Planner.png" width="95%" />
...
5. **三大模型系列（Cosmos World Foundation Models）**：

Cosmos 世界基础模型（WFM）按能力定位分为三条产品线，均支持后训练（post-training）以适配下游物理 AI 场景。项目主页与代码仓库见 [nvidia-cosmos](https://github.com/nvidia-cosmos)。

**① Cosmos-Predict —— 未来状态预测（Future World State Generation）**

- **定位**：给定文本、图像或历史视频作为条件，生成未来若干秒的世界演化视频，是最"经典"的世界模型形态，直接对应 Dreamer/Sora 这类"想象下一帧"的用途。
- **输入 / 输出**：Text2World、Image2World、Video2World 三种条件形式，输出为多秒、多视角、物理合理的视频片段。
- **架构**：同时提供 Diffusion（DiT 骨干，画质更高、3D 一致性更好）与 Autoregressive（Token 序列预测，更适合长序列、交互式展开）两种实现；规模从轻量的 2B 到 14B 不等（Cosmos-Predict2 进一步升级了采样效率与一致性）。
- **用途**：机器人策略的滚动仿真（rollout）、自动驾驶的闭环场景合成（synthetic driving data）、数字孪生（digital twin）中的反事实（counterfactual）视频生成。

**② Cosmos-Transfer —— 世界到世界迁移（Structured Sim2Real / World-to-World Transfer）**

- **定位**：不是"凭空想象未来"，而是把一种"结构化世界表示"翻译成另一种"感官真实的世界"。典型做法是把仿真器输出的几何/语义信号（如分割图、深度图、边缘图、HD map、LiDAR 投影）逐帧转成照片级逼真视频。
- **关键能力**：**多控制信号融合与时空权重（spatiotemporal control weights）**，允许同一帧内不同区域、不同时间使用不同的控制输入（例如前景用分割图保主体一致，背景用深度图保几何），从而在风格迁移与物理一致性之间精细平衡。
- **用途**：把 Isaac Sim / CARLA 等仿真数据"升级"为照片级训练数据，缓解 Sim2Real Gap；从单一场景生成大规模、多样化、可控的合成训练集（天气、光照、材质随意切换）；以及对已有真实视频做可控编辑（如替换物体、改变时刻）。
- **与 Predict 的区别**：Predict 关心"下一步发生什么"（时间外推），Transfer 关心"同一件事看起来像什么"（模态/外观迁移）。

**③ Cosmos-Reason —— 物理推理视觉语言模型（Physical-AI Reasoning VLM）**

- **定位**：一个面向物理 AI 的视觉语言模型（VLM），以视频/图像 + 文本为输入，输出带 **思维链（Chain-of-Thought, CoT）** 的自然语言回答，专门针对"这段视频里物理上发生了什么、接下来可能发生什么、机器人应该怎么做"类问题进行强化。
- **训练范式**：在常规多模态预训练之上，使用 **物理常识与具身推理（Physical Common Sense + Embodied Reasoning）** 数据进行 SFT，并通过 RL（可验证奖励）进一步加强对因果、碰撞、可行性等物理约束的遵循。
- **用途**：给 Predict/Transfer 生成的视频当"裁判"（自动化评估物理合理性）、作为机器人/VLA 系统的高层规划器（task decomposition、affordance reasoning）、以及作为数据策展流水线中的自动打标与过滤模块。
- **与前两者的关系**：Predict 负责"生成"世界，Transfer 负责"翻译"世界，Reason 负责"理解与判断"世界；三者在 Cosmos 平台里形成闭环——Reason 可以评估 Predict/Transfer 的输出质量，并反过来指导它们的数据筛选与后训练。

| 模型 | 任务形式 | 典型输入 | 典型输出 | 关键价值 |
| --- | --- | --- | --- | --- |
| Cosmos-Predict | 未来预测 / 视频生成 | Text / Image / 历史视频 | 未来多秒视频 | 作为可滚动仿真器供策略训练 |
| Cosmos-Transfer | 结构化→感知迁移 | Seg / Depth / Edge / LiDAR / HD map | 照片级真实视频 | 大规模可控合成数据、Sim2Real |
| Cosmos-Reason | 物理推理 VLM | 视频 + 文本问题 | 带 CoT 的自然语言回答 | 物理常识评估、具身规划、数据策展 |

6. **Cosmos Cookbook —— 官方落地食谱（Recipes & Tutorials）**

官方仓库 [nvidia-cosmos/cosmos-cookbook](https://github.com/nvidia-cosmos/cosmos-cookbook) 是配合论文与模型权重发布的"使用手册"，补齐了论文里没展开的工程落地环节。其定位是**给研究者和工程师一份可直接跑通的端到端 recipes 集合**，涵盖从环境搭建、推理调用、后训练（post-training）到数据策展的完整链路。

主要内容：

- **推理示例（Inference Recipes）**：针对 Predict（Text2World / Image2World / Video2World）、Transfer（多控制信号融合 + 时空权重）、Reason（视频问答 + CoT）三条产品线分别提供最小可运行脚本，方便快速验证模型能力。
- **后训练模板（Post-training Recipes）**：给出在 Predict 上做**相机控制（Camera Control）、机器人操纵（Robotic Manipulation）、自动驾驶（Driving）**等下游任务微调的标准配置，包含数据格式、LoRA/全量微调策略、训练超参。
- **数据策展（Video Curator）**：演示如何使用 Cosmos Video Curator 流水线对原始视频做过滤、切分、打标，以便自定义数据集进入后训练流程。
- **评估与 Guardrail**：提供用 Cosmos-Reason 作为"物理合理性裁判"的自动评估脚本，以及 Guardrail 安全过滤的调用示例。
- **部署工具链**：和 NVIDIA NeMo、Isaac Sim、TensorRT-LLM 等生态的集成示例，降低从科研原型到生产部署的迁移成本。

对我们而言，Cookbook 的意义是**把 Cosmos 从"论文里的平台"变成"可复现的基线"**：做 VLA / 世界模型相关工作时，可以直接拿 Predict 做 rollout 仿真、拿 Transfer 生成大规模 Sim2Real 数据、拿 Reason 做自动化评估，而不必从零复现论文里的训练与推理流程。
...
## 4.10 Video Generation Models in Robotics (2026)
