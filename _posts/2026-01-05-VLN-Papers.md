...
author: Tingde Liu
toc: true
excerpt: "阿里 AMAP CV Lab 在 OpenClaw 运行时上构建的具身 Agent 框架，用统一具身接口、视觉中心多模态记忆与 critic 闭环反馈三件套打通异构机器人长程协作。"
---
...
## 41. AdaOcc (2025)
———（Adaptive 3D Occupancy Prediction for Embodied Tasks） Point-Based Adaptive Perception with Heterogeneous Sensors

📄 **Paper**: [NeurIPS 2026 Submission] https://adaocc.online/

---

### 精华

1. **自适应点查询是感知-部署解耦的关键**：AdaOcc 用可变数量的稀疏点查询替代固定体素网格，让同一个训练好的模型在推理时仅需改变查询数量或提前终止解码层，无需重训练即可适配不同计算预算——这是"一次训练、多点部署"的核心设计原则。
2. **几何统一接口消除传感器绑定**：将深度图、RGB-D 相机和 LiDAR 统一归一化为同一坐标系下的点云，使双分支编码器对几何来源不感知，从而让模型泛化到不同传感器组合，而无需针对每种硬件分别训练。
3. **包含损失（Containment Loss）是稀疏点监督的关键补充**：Chamfer Distance 只鼓励预测点"靠近"真实面，但不保证点在已占用体素内部，导致漂浮伪影；包含损失只对落入空白监督区域的点施加惩罚，精准消除表面漂移而不压制有效预测。
4. **渐进式查询课程训练提升跨预算泛化**：从少量查询开始、逐渐增加到最大预算的训练策略，让模型学会在稀疏查询下也能产生合理预测，使推理时任意中间查询数量都可用。
5. **点集表示是稀疏场景理解的高效路径**：相比稠密体素（内存 ∝ $H \times W \times D$），预测稀疏语义点集再体素化仅需关注占用区域，在室内具有天然稀疏优势，且与下游任务（路径规划、操作定位）的接口更直接。

---

### 1. 研究背景/问题

具身 AI 任务（导航、操作、移动操作）依赖精确、灵活、语义丰富的 3D 场景表示。3D 语义占用预测（Semantic Occupancy Prediction）通过同时编码几何占用与语义类别来建模完整 3D 空间，是具身感知的理想表示。然而，现有方法存在三大实际部署瓶颈：**①** 无法适配异构传感器（有些平台无 LiDAR，有些只有单目，有些有多目）；**②** 无法自适应计算预算（不同机器人算力差异悬殊，不同任务所需感知粒度也不同）；**③** 大多基于固定视角设置训练，跨平台泛化能力有限。
...
AdaOcc 由三个核心模块构成：**自适应几何引导双分支编码器**（Adaptive Geometry-Guided Dual-Branch Encoder）、**渐进式点查询解码器**（Progressive Point Query Decoder）、以及**包含引导点优化**（Containment-Guided Point Optimization，C.P.O.）。编码器将异构传感器输入统一处理，解码器生成稀疏语义点集，C.P.O. 通过包含损失约束点的空间合法性。

<div align="center">
  <img src="/images/vla/AdaOcc-architecture.png" width="100%" />
<figcaption>图 2：AdaOcc 架构图——左侧双分支编码器分别提取图像特征和几何特征，中间渐进式解码器通过多层点查询迭代细化，训练时附加包含损失（Containment Loss）约束点的空间合法性。</figcaption>
</div>

**② 自适应几何引导双分支编码器**

- **输入**：多视角 RGB 图像 $$I = \{I_v\}_{v=1}^V$$，以及可选的几何输入（估计深度图、深度相机、LiDAR 点云）
- **几何统一化**：将所有几何来源归一化为局部智能体坐标系下的点云 $$G = \{g_m\}_{m=1}^M$$。深度图通过相机内参反投影后用外参变换；LiDAR 点云通过标定外参变换到 RGB 相机坐标系
- **图像分支**：标准多尺度图像编码器（默认 C-RADIOv3-B），输出多尺度特征图 $$F^{\text{img}} = \{F_r^{\text{img}}\}_{r=1}^{R_{\text{img}}}$$
- **几何分支**：将 $$G$$ 体素化后通过稀疏 3D 卷积编码器处理，再压缩到三个正交特征平面 $$\{F_{xy}, F_{xz}, F_{yz}\}$$（xy 平面捕获水平布局，xz/yz 平面保留垂直结构轮廓）
- **设计动机**：三平面压缩避免了稠密 3D 特征体的高内存开销，同时保留高度感知的空间上下文

**③ 渐进式点查询解码器**

解码器受 OPUS 启发，关键扩展是几何引导初始化和渐进式查询学习：

- **查询初始化**：从几何点云 $$G$$ 中采样 $$N_q$$ 个 3D 位置作为初始查询锚点 $$Q^0 = \{(q_i^0, f_i^0)\}_{i=1}^{N_q}$$，提供传感器无关的空间先验
- **每层解码**：每个查询在当前参考位置周围预测 $$K$$ 个 3D 采样坐标，同时从图像特征（多视角 cross-attention）和几何特征（三平面采样）中获取视觉与几何证据，通过残差更新查询特征：$$f_i^{(\ell)} = f_i^{(\ell-1)} + \Delta f_i^{(\ell-1)}$$
- **多点预测**：每个更新后的查询在层 $$\ell$$ 预测 $$N_p^{(\ell)}$$ 个语义点 $$\{(\hat{p}_{i,j}^{(\ell)}, \hat{s}_{i,j}^{(\ell)})\}$$，下一层参考位置由预测点均值重新定位，实现迭代对齐
- **预算自适应**：推理时可通过改变 $$N_q$$（查询数量）或提前选用中间层输出来灵活控制计算量

**渐进式查询课程训练**（视角遮蔽 + 查询渐增）：

$$N_q(t) = \min\!\left(N_{\max},\ N_{\text{init}} + \left\lfloor \frac{t - t_0}{\Delta t} \right\rfloor \Delta N\right)$$

训练时从 $$N_{\text{init}}$$ 起步，每 $$\Delta t$$ 轮增加 $$\Delta N$$ 查询，直到 $$N_{\max}$$；同时随机 mask 部分视角，使模型能处理不同视角数量。

**④ 包含引导点优化（C.P.O.）**

<div align="center">
  <img src="/images/vla/AdaOcc-CPO.png" width="80%" />
<figcaption>图 3：C.P.O. 示意图——纯 Chamfer 损失会产生在空白区域漂浮的表面伪影；C.P.O. 通过包含损失将异常点约束进最近已占用体素内部，减少边界漂浮现象。</figcaption>
</div>

Chamfer Distance 只鼓励预测点接近真实点，但不强制点落在已占用体素内。C.P.O. 通过以下包含损失解决这一问题：

对预测点 $$\hat{p}$$ 到最近已占用体素中心 $$p_i \in P^+$$ 的距离定义为：

$$d_{\text{box}}(\hat{p}, P^+) = \min_{p_i \in P^+} \left\lVert \text{ReLU}\!\left(\lvert \hat{p} - p_i \rvert - \frac{v}{2}\right) \right\rVert_2$$

若 $$\hat{p}$$ 在任意已占用体素盒内，距离为零；越偏向空白区域，距离越大。

包含损失仅对落入**监督空白区域**的预测点施加惩罚：

$$\mathcal{L}_{\text{con}} = \frac{1}{|S|} \sum_{\ell \in S} \frac{1}{|\mathcal{O}^{(\ell)}|} \sum_{\hat{p} \in \mathcal{O}^{(\ell)}} \text{SmoothL1}\!\left(d_{\text{box}}(\hat{p}, P^+) + m\right)$$

其中 $$m$$ 是小正偏移，使损失在体素盒边界附近也保持激活，进一步驱使预测点进入占用区域内部。

**⑤ 整体训练目标**

$$\mathcal{L} = \sum_{\ell=1}^{L} \lambda^{L-\ell} \left( \lambda_{\text{sem}} \mathcal{L}_{\text{sem}}^{(\ell)} + \lambda_{\text{pts}} \mathcal{L}_{\text{pts}}^{(\ell)} + \lambda_{\text{con}} \mathcal{L}_{\text{con}} \right)$$

- $$\mathcal{L}_{\text{sem}}$$：Focal Loss，监督语义 logit
- $$\mathcal{L}_{\text{pts}}$$：Density-aware Chamfer Distance（DCD），监督点集重建
- $$\mathcal{L}_{\text{con}}$$：包含损失，约束点的空间合法性

---

### 3. 核心结果/发现

**Occ-ScanNet 主实验**：AdaOcc 以 **59.67 mIoU / 65.29 IoU** 达到新的 SOTA，超过前最优方法 SplatSSC **+7.84 mIoU / +2.46 IoU**。使用同等 EfficientNet 骨干的受控对比实验（AdaOcc†）仍比 SplatSSC 高 +7.20 mIoU / +1.77 IoU，证明性能提升主要来自方法本身而非骨干差异。

<div align="center">
  <img src="/images/vla/AdaOcc-results.png" width="100%" />
<figcaption>图 4：Occ-ScanNet 定性对比——AdaOcc 生成的语义占用在物体边界和室内布局完整性方面明显优于 SplatSSC。</figcaption>
</div>

**效率-精度权衡**（与 SplatSSC 受控对比，单 NVIDIA H20 GPU）：
- AdaOcc 500 查询：57.97 mIoU，65.16 IoU，**64.4 ms**，1710.6 MiB
- SplatSSC 最优：48.46 mIoU，60.86 IoU，**85.1 ms**，1826.1 MiB

AdaOcc 更快、更精确、更省显存。通过改变解码层数还可进一步压缩延迟（仅 6 层时约 64.4 ms，3 层时约 58.2 ms），同一训练模型无需重训即可选用中间层输出。

**消融分析**（Occ-ScanNet-mini）：
- 基线（RADIO 编码器）：49.65 mIoU / 56.29 IoU
- +C.P.O. + 渐进训练：55.50 mIoU / 62.92 IoU（+5.85 / +6.63）
- +几何初始化：57.06 mIoU / 64.72 IoU（+1.56 / +1.80）
- +3D 编码器（完整）：58.74 mIoU / 65.38 IoU（+1.68 / +0.66）

**Occ3D-nuScenes 户外实验**：AdaOcc 以 RGB+伪深度达 44.97 RayIoU，加入 LiDAR 后达 50.89 RayIoU，超过所有先前方法（最优对比 OPUS-L 41.2 RayIoU）。

**真实机器人部署**：在 Unitree Go2（四足）+ ARX X5（轮臂）机器人上完成导航、操作、移动操作三类任务，验证 AdaOcc 作为实际部署感知模块的可行性。

---

### 4. 局限性

当前方法在杂乱场景中对**小型和细长物体**（如铅笔、细杆）的重建效果有限，因为稀疏点查询在高频细节上的覆盖能力不足；未来工作将专注于改进此类目标的几何建模。
...
## 10. Video Generation Models in Robotics (2026)
...
38. **GSMem** (2025). 3D Gaussian Splatting as Persistent Spatial Memory for Zero-Shot Embodied Exploration and Reasoning. arXiv: [2603.19137](https://arxiv.org/abs/2603.19137)
...
33. **Diffusion Policy** (2023). Visuomotor Policy Learning via Action Diffusion. arXiv: [2303.04137](https://arxiv.org/abs/2303.04137)
34. **VLN-CE** (2020). Beyond the Nav-Graph: Vision-and-Language Navigation in Continuous Environments. arXiv: [2004.02857](https://arxiv.org/abs/2004.02857)
35. **VLN-PE** (2025). Vision-Language Navigation in Physical Embodied Environments. arXiv: [2507.13019](https://arxiv.org/abs/2507.13019)
36. **Qwen3-VL** (2025). arXiv: [2511.21631](https://arxiv.org/abs/2511.21631)
37. **Lumina-Next** (2024). Next-DiT Scaling for Next-Generation Image Generation. arXiv: [2406.18583](https://arxiv.org/abs/2406.18583)
38. **InternVLA-A1** (2026). arXiv: [2601.02456](https://arxiv.org/abs/2601.02456)
39. **InternData-A1** (2025). arXiv: [2511.16651](https://arxiv.org/abs/2511.16651)
40. **RynnBrain** (2026). Open Embodied Foundation Models. arXiv: [2602.14979](https://arxiv.org/abs/2602.14979)
41. **Janus-Pro** (2025). Unified Multimodal Understanding and Generation with Data and Model Scaling. arXiv: [2501.17811](https://arxiv.org/abs/2501.17811)
42. **Video Generation Models in Robotics** (2026). Applications, Research Challenges, Future Directions. arXiv: [2601.07823](https://arxiv.org/abs/2601.07823)
43. **LangSplatV2: High-dimensional 3D Language Gaussian Splatting with 450+ FPS** (2025). 彻底告别解码器：基于稀疏系数场的超高速 3D 语言场. arXiv: [2507.07136](https://arxiv.org/abs/2507.07136)
44. **OpenGaussian** (2024). 迈向点云级 3D Gaussian 开放词汇理解. arXiv: [2406.02058](https://arxiv.org/abs/2406.02058)
45. **LingBot-World: Advancing Open-source World Models** (2026). 首个开源、支持实时交互的长程世界模型. arXiv: [2601.20540](https://arxiv.org/abs/2601.20540)
46. **WorldVLN** (2025). Autoregressive World Action Model for Aerial Vision-Language Navigation. arXiv: [2605.15964](https://arxiv.org/abs/2605.15964)
47. **WorldVLN** (2025). Autoregressive World Action Model for Aerial Vision-Language Navigation. arXiv: [2605.15964](https://arxiv.org/abs/2605.15964)
48. **AdaOcc: Adaptive 3D Occupancy Prediction for Embodied Tasks** (2025). Point-Based Adaptive Perception with Heterogeneous Sensors.
...
(function () {
  var TAG_MAP = [
    ...
    { m: 'ODYSSEY',           t: ['Agent', '实机部署'] },
    { m: 'PanoNav',           t: ['Agent', '零样本', '离散环境'] },
    ...
    { m: 'NavGPT-2',          t: ['Agent', '拓扑图', '离散环境'] },
    { m: 'NavGPT',            t: ['Agent', '零样本', '离散环境'] },
    ...
    { m: 'Open-Nav',          t: ['Agent', '零样本', '连续环境'] },
    { m: 'CausalNav',         t: ['Agent', '拓扑图'] },
    { m: 'SparseVideoNav',    t: ['端到端', '扩散模型', '世界模型'] },
    { m: 'AgentVLN',          t: ['Agent', '连续环境', '实机部署'] },
    { m: 'ABot-Claw',         t: ['Agent', '实机部署'] },
    { m: 'RoboClaw',          t: ['Agent'] },
    { m: 'Skill-Nav',         t: ['端到端', '强化学习', '实机部署'] },
    { m: 'VLN-Cache',         t: ['加速优化'] },
    { m: 'SysNav',            t: ['Agent', '拓扑图'] },
    { m: 'VLN-Imagine',       t: ['数据增强', '离散环境'] },
    { m: 'Uncertainty-Aware', t: ['高斯表示', '拓扑图', '离散环境'] },
    { m: 'GSMem',             t: ['Agent', '高斯表示', '零样本'] },
    { m: 'R³',                t: ['双系统', '加速优化'] },
    { m: 'LangSplatV2',       t: ['高斯表示', '加速优化', '基础工作'] },
        { m: 'OpenGaussian',          t: ['高斯表示', '零样本', 'SLAM'] },
        { m: 'LingBot-World: Advancing Open-source World Models', t: ['世界模型', '扩散模型', 'Agent', '加速优化'] },
        { m: 'WorldVLN',              t: ['世界模型', '强化学习', '端到端', '实机部署'] },
        { m: 'WorldVLN',              t: ['世界模型', '强化学习', '端到端', '实机部署'] },
        { m: 'AdaOcc: Adaptive 3D Occupancy Prediction for Embodied Tasks', t: ['基础工作', '实机部署', '连续环境'] },
    { m: 'Diffusion Policy',  t: ['扩散模型', '基础工作'] },
    { m: 'Lumina-Next',       t: ['扩散模型', '基础工作'] },
    { m: 'Qwen3-VL',          t: ['基础工作'] },
    { m: 'VLN-CE',            t: ['数据集', '连续环境', '基础工作'] },
    { m: 'VLN-PE',            t: ['数据集', '连续环境', '基础工作'] },
    { m: 'InternVLA-A1',      t: ['世界模型', '基础工作'] },
    { m: 'InternData-A1',     t: ['数据集', '数据增强', '基础工作'] },
    { m: 'RynnBrain',         t: ['基础工作'] },
    { m: 'Janus-Pro',         t: ['世界模型', '基础工作'] },
    { m: 'Video Generation',  t: ['世界模型', '基础工作'] }
  ];

  var ALL_TAGS = ['双系统', '端到端', 'Agent', '扩散模型', '拓扑图', 'SLAM', '高斯表示',
                  '强化学习', '零样本', '世界模型', '数据增强',
