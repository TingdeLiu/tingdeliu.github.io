...
节点是 ROS 2 的**最小功能单元**，每个节点负责一件具体的事情（传感器读取、路径规划、电机控制等），节点之间通过话题/服务/动作通信。
...
## 10.1 生命周期节点（Lifecycle Node）

对于需要**有序启动**的系统（如：必须等传感器初始化后才能启动规划器），普通节点无法保证顺序。生命周期节点引入了确定性状态机：
